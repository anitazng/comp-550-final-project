...
def generate_character_embeddings(datafile):
    ...
    for i in range(1, 200):
        new_md = Word2Vec(tokenized_sentences,min_count=40, vector_size=300,epochs=i, compute_loss = True)
        loss = new_md.get_latest_training_loss()
        prev_loss = curr_loss
        curr_loss = loss
        if i != 1:
            actual = curr_loss - prev_loss
            print("the loss for epoch ", i, " is ", actual )
        
    model = Word2Vec(tokenized_sentences, vector_size=300,epochs=15, compute_loss = True)
            ...
            for h1 in homophones: # compute average homophone pair distance and baseline distance for each homophone
                for h2 in homophones:
                    if h1 != h2:
                        ...
                        # compute homophone similaritiescd
...
if __name__ == "__main__":
    ...
    print(embeddings.get_latest_training_loss())
